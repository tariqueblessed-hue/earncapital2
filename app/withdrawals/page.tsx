...
export default function WithdrawPage() {
  ...
  async function loadData() {
    ...
    if (referrals) {
      ...
      setActiveReferrals(
        referrals.filter((r: any) => r.fee_paid).length
      );
    }
  ...
  async function submitWithdrawal() {
    ...
    if (withdrawAmount < 1000) {
      alert("❌ Minimum withdrawal is KES 1,000.");
      return;
    }
    ...
    >
      ...
      >
        ...
        >
          {method === "M-Pesa" && (
            <>
              <p><strong>📱 Number:</strong> {mpesaNumber || "Not saved"}</p>
              <p><strong>👤 Name:</strong> {mpesaName || "Not saved"}</p>
            </>
          )}

          {method === "PayPal" && (
            <p><strong>💵 Email:</strong> {paypalEmail || "Not saved"}</p>
          )}

          {method === "Bank" && (
            <>
              <p><strong>🏦 Bank:</strong> {bankName || "Not saved"}</p>
              <p><strong>💳 Account:</strong> {bankAccount || "Not saved"}</p>
              <p><strong>👤 Name:</strong> {bankAccountName || "Not saved"}</p>
            </>
          )}
        </div>
